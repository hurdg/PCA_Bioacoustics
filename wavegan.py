...
####################################
## Changed line 16,33,68, 69, 70, 72, 77,85,93,100,109,115,122,128 (tf.compat.v1)
...
def WaveGANGenerator(
  ...
  with tf.compat.v1.variable_scope('z_project'):
    ...
    output = tf.compat.v1.reshape(output, [batch_size, 16, dim * dim_mul])
...
def lrelu(inputs, alpha=0.2):
  return tf.compat.v1.maximum(alpha * inputs, inputs)
...
def apply_phaseshuffle(x, rad, pad_type='reflect'):
  ...
  phase_start = pad_r
  x = tf.compat.v1.pad(x, [[0, 0], [pad_l, pad_r], [0, 0]], mode=pad_type)
...
def WaveGANDiscriminator(
  ...
  # Flatten
  output = tf.compat.v1.reshape(output, [batch_size, -1])
